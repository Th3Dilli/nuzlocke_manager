...
function OverlayInner({token}: { token: string }) {
    ...
    return (
        <div className="flex h-[1080px] w-[1920px] justify-between gap-2 p-2 overlay">
            ...
            <div className="flex flex-1 flex-col gap-4" style={{maxWidth: `${state.sideMaxWidth}px`}}>
                {getLayout(state)}
                <TeamBox team={state.team} label={state.showTeamLabel ? state.teamLabel : ""} color={state.teamColor} textColor={state.textColor} className="mt-auto h-24"/>
